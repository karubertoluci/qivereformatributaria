  ...
  return (
    <div className="bg-secondary p-4 rounded-lg mb-6">
      ...
      {topicsToShow.length > 0 && (
        <div className="mt-3">
          <h4 className="text-xs font-medium text-gray-500 mb-1.5">Principais tópicos:</h4>
          <div className="flex flex-wrap gap-2">
            {topicsToShow.map(topic => (
              <div key={topic.id} className="text-xs bg-background rounded-full px-3 py-1">
                {topic.name}
              </div>
            ))}
            {topics.length > 3 && (
              <div className="text-xs bg-background rounded-full px-3 py-1 text-muted-foreground">
