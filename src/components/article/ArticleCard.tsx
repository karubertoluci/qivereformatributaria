...
const ArticleCard: React.FC<ArticleCardProps> = ({
  ...
  if (random < 40) {
    ...
    if (hasNegativeImpact) primaryImpactType = 'negative';else if (hasPositiveImpact) primaryImpactType = 'positive';
  }
  ...
  return <Card className={`mb-4 border-l-4 hover:shadow-md transition-all ${primaryImpactType === 'positive' ? 'border-l-green-500' : primaryImpactType === 'negative' ? 'border-l-red-500' : 'border-l-gray-300'}`}>
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          ...
          <Badge variant={primaryImpactType === 'positive' ? 'default' : primaryImpactType === 'negative' ? 'destructive' : 'secondary'} className={primaryImpactType === 'positive' ? 'bg-green-500' : primaryImpactType === 'negative' ? 'bg-red-500' : ''}>
            {primaryImpactType === 'positive' ? 'Favorável' : primaryImpactType === 'negative' ? 'Desfavorável' : 'Neutro'}
          </Badge>
        </div>
        
        <div className="flex items-center text-xs text-muted-foreground mt-1">
          <BookOpen className="h-3.5 w-3.5 mr-1" />
          <span className="text-left">{getBookInfo()}, Artigo {article.number || 'N/A'}</span>
        </div>
      </CardHeader>
      
      <CardContent>
        <p className="text-sm line-clamp-3 mb-2 text-left">
          {article.simplifiedText || 'Sem texto disponível'}
        </p>
        
        <div className="flex flex-wrap gap-2 mt-2">
          <Badge variant="outline" className={`text-xs ${relevanceText === 'Muito relevante' ? 'bg-green-50 text-green-700 border-green-200' : relevanceText === 'Moderadamente relevante' ? 'bg-amber-50 text-amber-700 border-amber-200' : relevanceText === 'Pouco relevante' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-gray-50 text-gray-700 border-gray-200'}`}>
            {relevanceText}
          </Badge>
          
          {article.metadata?.titulo && <Badge variant="outline" className="text-xs">
      ...
      <CardFooter>
        <Dialog>
          ...
          <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
            ...
            <ArticleContent article={article} segmentId={segmentId} highlights={highlights} onAddHighlight={onAddHighlight} onRemoveHighlight={onRemoveHighlight} />
          </DialogContent>
        </Dialog>
      </CardFooter>
    </Card>;
