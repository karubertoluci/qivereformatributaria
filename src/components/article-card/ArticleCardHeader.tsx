...
const ArticleCardHeader: React.FC<ArticleCardHeaderProps> = ({ 
  ...
  const getImpactType = () => {
    ...
    if (randomImpact < 40) return { type: 'positive', label: 'Favorável', color: 'bg-green-100 text-green-700' };
    if (randomImpact < 60) return { type: 'neutral', label: 'Neutro', color: 'bg-gray-100 text-gray-700' };
    return { type: 'negative', label: 'Desfavorável', color: 'bg-red-100 text-red-700' };
  };
  ...
  const getRelevanceInfo = () => {
    ...
    if (randomRelevance < 40) {
      return { label: 'Irrelevante', color: 'bg-green-100 text-green-700' };
    } else if (randomRelevance < 50) {
      return { label: 'Pouco relevante', color: 'bg-yellow-100 text-yellow-700' };
    } else if (randomRelevance < 90) {
      return { label: 'Moderadamente relevante', color: 'bg-orange-100 text-orange-700' };
    } else {
      return { label: 'Muito relevante', color: 'bg-red-100 text-red-700' };
    }
  };
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      {/* Metadados: Livro, Capítulo, Seção, Subseção */}
